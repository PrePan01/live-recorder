...
const devDataDir =
  process.env.LIVE_RECORDER_DATA_DIR ?? path.join(root, ".dev-data");
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      if (code === 0) resolve();
        ...
        reject(
          new Error(
            `${npm} ${args.join(" ")} ${signal ? `被 ${signal} 中断` : `退出，code=${code}`}`,
          ),
        );
    });
  });
...
console.log(
  `Tauri dev：后端=${adapter}，数据目录=${devDataDir}，端口=${devPort}`,
);
const occupiedPorts = (
  await Promise.all(
    [devPort, "5173"].map(async (port) =>
...
if (occupiedPorts.length > 0) {
  console.log(
    `检测到开发端口被占用（${occupiedPorts.join(", ")}），正在执行 npm run dev:stop…`,
  );
