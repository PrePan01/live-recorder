...
const ERROR_MATCHERS: Array<{ code: UploadErrorCode; test: RegExp; action: string }> = [
  ...
  { code: 'OPENLIST_AUTH_FAILED', test: /认证失败|401|403|Invalid.*(token|code|credential)/i, action: 'OpenList 认证失败，请在设置中核对用户名与令牌（WebDAV 密码）后重新上传；若账号启用了 2FA，请在弹出的窗口中输入一次性验证码。' },
