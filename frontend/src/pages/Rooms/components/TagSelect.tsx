...
export default function TagSelect({
  ...
  const onCreate = async () => {
    ...
    try {
      ...
      message.error(
        e instanceof ApiError ? describeError(e.code, e.message) : "创建失败",
      );
    } finally {
      setCreating(false);
    }
          ...
          <Form.Item name="color" initialValue={PRESET_COLORS[0]} noStyle>
            <Select
              ...
              options={PRESET_COLORS.map((c) => ({
                value: c,
                label: (
                  <AntTag className="lr-tag-color-option" color={c}>
                    {c}
                  </AntTag>
                ),
              }))}
            ...
            return (
              ...
              >
                <AntTag
                  ...
                  aria-label={
                    isSelected ? `${t.name} 已添加` : `添加标签 ${t.name}`
                  }
                  style={{
