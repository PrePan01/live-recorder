...
export default function EmailConfigCard() {
  ...
  const [email, setEmail] = useState<Awaited<
    ReturnType<typeof fetchEmail>
  > | null>(null);
      ...
      .catch((err) =>
        message.error(
          err instanceof ApiError
            ? describeError(err.code, err.message)
            : "邮件配置加载失败",
        ),
      );
  }, [form, message]);
  ...
  const onTest = async () => {
    ...
    try {
      ...
      message.error(
        e instanceof ApiError ? describeError(e.code, e.message) : "发送失败",
      );
    } finally {
      setTesting(false);
    }
  ...
  return (
    ...
    >
      ...
      <Form.Item
        label="密码"
        name="password"
        extra={email?.passwordSet ? "已保存，留空则不修改" : undefined}
      >
        <Input.Password
          placeholder={email?.passwordSet ? "••••••" : "输入 SMTP 密码"}
