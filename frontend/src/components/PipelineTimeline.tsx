...
export default function PipelineTimeline({ recordingId }: { recordingId: string }) {
  ...
  const audioOff = (run.run.configSnapshot as { exportAudio?: boolean } | null)?.exportAudio === false;
  if (!hasAudio && audioOff) {
    ...
    const insertAt = segIdx >= 0 ? segIdx + 1 : cmpIdx >= 0 ? cmpIdx : artifacts.length;
...
function ArtifactItem({ artifact }: { artifact: PipelineArtifact }) {
  ...
  const statusText = artifact.step === 'audio' && artifact.status === 'skipped' ? '未启用' : artifact.status;
  return (
    <div>
      <Space size={8} wrap>
        ...
        {artifact.step === 'audio' && artifact.status === 'ok' && artifact.path ? (
          <Button
            size="small"
            type="link"
