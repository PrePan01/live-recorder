      ...
      if (result.retainedFiles.length > 0) {
        ...
        modal.warning({
          ...
          content: (
            <div>
              以下临时文件仍在磁盘中，可手动清理：
              {result.retainedFiles.map((file) => (
                <div key={file} style={{ overflowWrap: "anywhere" }}>
                  {file}
                </div>
              ))}
            </div>
  ...
  return (
    <>
      <Card
        ...
        style={{
          marginTop: 16,
          borderTopColor: "var(--lr-danger) !important",
        }}
      >
        <Space orientation="vertical">
          <Typography.Text type="secondary">
            清空应用数据与凭证，恢复至初始设置。此操作无法撤销。
          </Typography.Text>
          <Button
            danger
            icon={<ReloadOutlined />}
      ...
      >
        <Space orientation="vertical" size={20} style={{ width: "100%" }}>
          <Alert
            type="warning"
            showIcon
            title="将清空所有房间、录制历史、标签、定时计划、任务记录、告警、设置与凭证，并恢复界面偏好。"
          />
          <Space wrap>
          ...
          <Typography.Text type="secondary">
            导出包含当前配置、房间、告警、录制历史数据与开播预测记录，不包含录像文件或密码/Cookie/令牌。凭证恢复后需重新填写。
          </Typography.Text>
          <Checkbox
            checked={keepRecordings}
            disabled={resetting}
            onChange={(event) => setKeepRecordings(event.target.checked)}
          >
