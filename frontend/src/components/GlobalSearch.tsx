...
function itemTitle(item: SearchItem): string {
  if (item.type === "recording")
    return item.extra?.roomName
      ? `${String(item.extra.roomName)} · ${item.title}`
      : item.title;
  return item.title;
}
  ...
  return (
    ...
    >
      <Input.Search
        ...
        onBlur={() => setTimeout(() => !q && setOpen(false), 50)}
      />
      {open ? (
        <div
          style={{
          ...
          {q.trim().length < 1 ? (
            ...
            <div ref={listRef} style={{ maxHeight: 320, overflow: "auto" }}>
              <List
                ...
                dataSource={results}
                locale={{
                  emptyText: total > 0 ? "无匹配结果" : "未找到相关内容",
                }}
                renderItem={(item, i) => (
                  <List.Item
                    data-idx={i}
                    style={{
                      ...
                      padding: "6px 12px",
                      background:
                        i === activeIdx ? "var(--lr-hover-bg)" : "transparent",
                    }}
              ...
              {total > results.length ? (
                <Typography.Paragraph
                  type="secondary"
                  style={{ padding: "4px 12px 8px", margin: 0, fontSize: 12 }}
                >
