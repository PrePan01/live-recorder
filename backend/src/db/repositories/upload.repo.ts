...
interface UploadJobRow {
  ...
  delete_source_after_success: number;
  created_at: string;
  updated_at: string;
}
...
function rowToJob(row: UploadJobRow): UploadJob {
  return {
    ...
    deleteSourceAfterSuccess: row.delete_source_after_success === 1,
...
export class UploadRepository {
  ...
  create(input: { recordingId: string; idempotencyKey: string; deleteSourceAfterSuccess?: boolean }): UploadJob | null {
    const id = newId('upl');
    const now = nowIso();
    ...
    const info = this.db
      .prepare(
        `INSERT OR IGNORE INTO upload_jobs (id, recording_id, status, progress, remote_path, error, retry_count, idempotency_key, delete_source_after_success, created_at, updated_at)
         VALUES (?, ?, 'queued', 0, NULL, NULL, 0, ?, ?, ?, ?)`,
      )
      .run(id, input.recordingId, input.idempotencyKey, input.deleteSourceAfterSuccess ? 1 : 0, now, now);
