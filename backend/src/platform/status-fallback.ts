...
const unknownCounter = new Map<
  string,
  { count: number; lastSeen: string; hint: string | null }
>();
export function unknownStatusSnapshot(): Array<{
  key: string;
  count: number;
  lastSeen: string;
  hint: string | null;
}> {
  return [...unknownCounter.entries()].map(([key, v]) => ({
    key,
    count: v.count,
    lastSeen: v.lastSeen,
    hint: v.hint,
...
export function unknownStatusFallback(opts: {
  ...
  unknownCounter.set(ckey, {
    count: (prev?.count ?? 0) + 1,
    lastSeen: new Date().toISOString(),
    hint: (hint ?? prev?.hint ?? null)?.slice(0, 160) ?? null,
  });
...
export function familyBySemantics(opts: {
  ...
  if (/可见范围|不在主播设置|不在.{0,4}范围|无法进入.{0,6}直播间/.test(hint)) {
    return new AppError("ROOM_CONTENT_UNAVAILABLE", "该直播间当前无法查看", {
      retryable: true,
      details,
    });
  }
  ...
  if (
    /服务繁忙|系统繁忙|busy|service unavailable|服务器(器)?(错误|过载|繁忙)|系统异常|internal server/i.test(
      hint,
    )
  ) {
    return new AppError("PLATFORM_CHANGED", "平台暂时繁忙，请稍后重试", {
      retryable: true,
      details,
    });
  }
  if (
    /未登录|请先登录|登录已失效|风控|安全验证|captcha|授权.*(失效|过期|无效)|login required/i.test(
      hint,
    )
  ) {
    return new AppError(
