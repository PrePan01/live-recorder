...
export function registerDiagnosticRoutes(
  ...
  /** Recent local-only timing traces are a development-only surface. */
  if (services.manager.performance.enabled) {
    app.get("/api/v1/diagnostics/performance", async (_req, reply) =>
      reply.send({ items: services.manager.performance.recent() }),
    );
  }
  ...
  app.get("/api/v1/diagnostics", async (req, reply) => {
    ...
    if (status !== undefined && !STATUSES.includes(status)) {
      throw new AppError(
        "CONFIG_INVALID",
        "status 仅支持 open/processing/resolved/expired",
      );
    }
    ...
    const olderThan = new Date(
      services.clock.now() - 30 * 24 * 60 * 60 * 1000,
    ).toISOString();
  ...
  app.get("/api/v1/diagnostics/:id", async (req, reply) => {
    ...
    if (!diagnostic)
      throw new AppError("RESOURCE_NOT_FOUND", "诊断项不存在", {
        details: { resource: "diagnostic" },
      });
    return reply.send({
      diagnostic,
      actions: services.diagnostics.actionsFor(id),
    });
  });
  ...
  app.post("/api/v1/diagnostics/:id/actions/:action", async (req, reply) => {
    ...
    if (diagnostic.status === "expired") {
      throw new AppError("DIAGNOSTIC_CONFLICT", "诊断项已过期，无法执行动作", {
    ...
    if (!allowed.includes(action)) {
      throw new AppError(
        ...
        `诊断 ${diagnostic.code} 不支持动作 ${action}`,
      );
    }
    ...
    const existing = services.diagnostics
      .actionsFor(id)
      .find((a) => a.idempotencyKey === idempotencyKey);
    if (existing) {
      return reply.send({
        diagnostic: services.diagnostics.get(id),
        action: existing,
      });
    }
    ...
    const nextStatus: DiagnosticStatus =
      performed.result === "ok"
        ...
        : diagnostic.status === "processing"
          ? "open"
          : diagnostic.status;
    services.diagnostics.setStatus(
      id,
      nextStatus,
      performed.result === "ok" ? services.clock.iso() : null,
    );
    ...
    services.events.emit({
      ...
      data: services.diagnostics.get(id)!,
    });
    return reply.send({
      diagnostic: services.diagnostics.get(id),
      action: record,
