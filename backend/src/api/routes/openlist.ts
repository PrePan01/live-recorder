...
export function registerOpenListRoutes(
  app: FastifyInstance,
  services: Services,
): void {
  ...
  app.put("/api/v1/settings/openlist", async (req, reply) => {
    const body = (req.body ?? {}) as Partial<OpenListConfig> & {
      token?: unknown;
    };
    const current = await openListView(services);
    const { token, ...rest } = body;
    if (
      typeof rest.enabled !== "undefined" &&
      typeof rest.enabled !== "boolean"
    ) {
      throw new AppError("CONFIG_INVALID", "enabled 必须为布尔值");
    }
    if (
      typeof rest.serverUrl !== "undefined" &&
      typeof rest.serverUrl !== "string"
    ) {
      throw new AppError("CONFIG_INVALID", "serverUrl 必须为字符串");
    }
    if (
      typeof rest.username !== "undefined" &&
      typeof rest.username !== "string"
    ) {
    ...
    const settings =
      ...
      ({ recordingDirectory: "" } as Record<string, unknown>);
    ...
    if (typeof token === "string") {
      if (token.length > 0)
        await services.secretStore.set(OPENLIST_TOKEN_KEY, token);
  ...
  app.post("/api/v1/settings/openlist/test", async (_req, reply) => {
    ...
    if (!config.serverUrl)
      throw new AppError("CONFIG_LOAD_FAILED", "OpenList 地址未配置");
    if (!config.hasToken)
      throw new AppError("CONFIG_LOAD_FAILED", "OpenList 令牌未配置");
    try {
      ...
      if (!res.ok) {
        ...
        throw new AppError("CONFIG_LOAD_FAILED", message, {
  ...
  app.post("/api/v1/settings/openlist/2fa", async (req, reply) => {
    ...
    if (!result.ok) {
      throw new AppError(
        "CONFIG_LOAD_FAILED",
        result.message ?? "OpenList 2FA 验证失败",
        { retryable: true },
      );
    }
    return reply.send({ ok: true });
  ...
  app.post("/api/v1/uploads/:id/retry", async (req, reply) => {
    ...
    if (!job)
      throw new AppError("RESOURCE_NOT_FOUND", "上传任务不存在", {
  ...
  app.post("/api/v1/recordings/:id/upload", async (req, reply) => {
    ...
    if (!rec.filePath || !existsSync(rec.filePath)) {
      throw new AppError("CONFIG_LOAD_FAILED", "源文件已删除，无法上传", {
        recordingId: id,
      });
    }
