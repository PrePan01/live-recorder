...
import { boundedDiagnosticLog, createDiagnosticBundle, redactDiagnosticText, roomDiagnosticSnapshots } from '../../src/api/diagnostic-bundle.js';
...
describe('diagnostic bundle privacy boundary', () => {
  ...
  it('keeps only public room identifiers and support-safe fields', () => {
    ...
    services.rooms.create({ platform: 'bilibili', url: 'https://live.bilibili.com/12345', displayName: '私密名称' });
    const rooms = roomDiagnosticSnapshots(services);
    expect(rooms).toEqual([expect.objectContaining({ platform: 'bilibili', publicRoomId: '12345' })]);
  ...
  it('caps logs at one MiB and retains their tail', () => {
    const oversized = Buffer.concat([Buffer.alloc(1_048_576, 'a'), Buffer.from('TAIL')]);
  ...
  it('creates every fixed ZIP entry even when both backend logs are absent', async () => {
    ...
    for (const entry of ['README.txt', 'runtime.json', 'rooms.json', 'frontend-errors.json', 'backend.log', 'backend.previous.log']) {
  ...
  it('omits every room entry when room diagnostics are declined', async () => {
    const services = buildServices({ dbPath: ':memory:', clock: new FakeClock() });
    services.rooms.create({ platform: 'douyin', url: 'https://live.douyin.com/12345', displayName: '不应导出' });
    const archive = await createDiagnosticBundle(services, {
      stateDir: '/private/tmp/live-recorder-no-diagnostic-logs',
      includeRooms: false,
    });
